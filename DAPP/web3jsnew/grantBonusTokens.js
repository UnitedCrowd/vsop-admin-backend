...
function grantBonusToken(employee_address,amount)
{
	if(!web3.isConnected()) {
	...
	crowdsaleInstance.grantBonusTokens.estimateGas(employee_address,amount, function(error, result) {
		if (!error)
		{
			crowdsaleInstance.grantBonusTokens(employee_address,amount, function(error, result)
			{
				if (!error)
				{
