function revokeVesting(beneficiary)
{
	if(!web3.isConnected()) {
	...
	crowdsaleInstance.revokeVesting.estimateGas(beneficiary, function(error, result) {
		if (!error)
		{
			crowdsaleInstance.revokeVesting(beneficiary, function(error, result)
			{
				if (!error)
				{
